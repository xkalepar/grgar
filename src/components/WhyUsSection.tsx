...
import { Target, Shield, Users, Award, Truck, HeartHandshake, Clock, MapPin, CheckCircle } from "lucide-react";
...
const stats = [
  { icon: Clock, value: "+١٥", label: "سنوات خبرة" },
  { icon: MapPin, value: "٣", label: "تغطية جغرافية" },
  { icon: CheckCircle, value: "٢٤/٧", label: "دعم متواصل" },
];
...
const WhyUsSection = () => {
  ...
  return (
    <section ref={containerRef} id="why-us" className="relative overflow-hidden">
            ...
            <div className="flex flex-wrap items-center justify-center gap-8 pt-8 mt-8 border-t border-primary-foreground/20">
              {stats.map((stat, index) => (
                ...
                >
                  ...
                  <span className="text-3xl font-black text-primary-foreground">{stat.value}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reasons.map((reason, index) => (
            <motion.div
              ...
              className="group p-6 rounded-2xl bg-primary-foreground/5 border border-primary-foreground/10 hover:bg-primary-foreground/10 hover:border-accent/30 transition-all duration-500 hover:-translate-y-1"
            >
              <div className="w-14 h-14 rounded-xl bg-accent/20 flex items-center justify-center mb-4 group-hover:bg-accent/30 group-hover:shadow-glow-accent transition-all duration-500">
